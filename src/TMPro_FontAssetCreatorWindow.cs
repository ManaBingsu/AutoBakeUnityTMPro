...
namespace TMPro.EditorUtilities
{
    public class TMPro_FontAssetCreatorWindow : EditorWindow
    {
        ...
        Queue<KeyValuePair<TMP_FontAsset, uint[]>> autoBakeQueue = new Queue<KeyValuePair<TMP_FontAsset, uint[]>>();

        public static void ShowFontAtlasCreatorWindow(KeyValuePair<TMP_FontAsset, uint[]> value)
        {
            var window = GetWindow<TMPro_FontAssetCreatorWindow>();
            TMP_FontAsset fontAsset = value.Key;
            uint[] characters = value.Value;
            if (!window.isAutomate)
            {
                window.titleContent = new GUIContent("Font Asset Creator");
                window.Focus();
                // Clear any previously generated data
                ...
                if (string.IsNullOrEmpty(fontAsset.creationSettings.sourceFontFileGUID) == false)
                {
                    ...
                    // Override settings to inject character list from font asset
                    window.m_CharacterSetSelectionMode = 7;
                    window.m_CharacterSequence = TMP_EditorUtility.GetUnicodeCharacterSequence(TMP_FontAsset.GetCharactersArray(fontAsset));
                ...
                window.TryAddCharactersInEditor(characters);
            }
            else
            {
                window.autoBakeQueue.Enqueue(value);
            }
        }
                ...
                if (m_IsGenerationCancelled == false)
                {
                    ...
                    if (isAutomate)
                    {
                        ...
                        if (m_MissingCharacters.Count > 0)
                        {
                            ...
                            foreach (uint code in m_MissingCharacters)
                            {
                                strBuild.Append(Convert.ToChar(code));
                                strBuild.Append(" ");
                            }
                            Debug.LogError("<color=red>[Fatal error]</color>(" + m_SelectedFontAsset.name + ") Missing string(" + m_MissingCharacters.Count + ") : " + strBuild.ToString());
                        }
                        ...
                        if (autoBakeQueue.Count > 0)
                        {
                            TMPro_FontAssetCreatorWindow.ShowFontAtlasCreatorWindow(autoBakeQueue.Dequeue());
                        }
                    }
                }
        ...
        // Custom function for generate atlas
        public void TryAddCharactersInEditor(uint[] characters)
        {
            isAutomate = true;
